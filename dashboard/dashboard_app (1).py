...
# On Streamlit Cloud, config is set via "Secrets" (TOML), not a .env file.
# This copies any secrets into os.environ so the existing app/*.py modules
# (which read via os.getenv) work unchanged locally AND on Streamlit Cloud.
try:
